...
export const getChatbotResponse = async (userMessage, language = 'hi') => {
  try {
    ...
    const prompt = `You are FarmGuru, an expert farming assistant for Indian farmers.

CRITICAL: Respond in ${detectedLang === 'hi' ? 'HINDI (Devanagari script)' : 'ENGLISH'}.

Answer this farmer question directly with practical advice (2-3 sentences):
...
export const getGovernmentSchemes = async (category, state) => {
  try {
    const prompt = `List 3 government schemes for farmers in ${state || 'India'}. Include scheme name, benefit, and how to apply. Response in simple Hindi.`;
    const response = await groq.chat.completions.create({
      messages: [{ role: "user", content: prompt }],
      model: "llama-3.3-70b-versatile",
      max_tokens: 300,
